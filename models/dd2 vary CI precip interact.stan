...
transformed parameters {
  ...
  for(s in 1:S) {
    ...
    for(t in 3:N) {
      x[t,s] = alpha_0 + alpha_1*x[t-1,s] + alpha_2[wetdry[s]]*x[t-2,s] + beta_precip*precip[t-1] + beta_temp*temp[t-1] + beta_CI*CI[s,t-1] + beta_pt*precip[t-1]*temp[t-1] + beta_CIp*precip[t-1]*CI[s,t-1];
      }
  }
...
model {
  ...
  for(w in 1:W) {
    alpha_2[w] ~ normal(0, 10); // direct density-dependence
   
  
  }
  alpha_0 ~ normal(0, 10); // intercepts
  alpha_1 ~ normal(0, 10); //delayed density-dependence
  ...
  beta_CIp ~ normal(0, 10); //connectivity precip interaction
  // likelihood
  for(i in 1:n_pos) {
