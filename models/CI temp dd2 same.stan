...
transformed parameters {
  ...
  for(s in 1:S) {
    ...
    for(t in 3:N) {
      x[t,s] = alpha_0[wetdry[s]] + alpha_1[wetdry[s]]*x[t-1,s] + alpha_2*x[t-2,s] + beta_precip[wetdry[s]]*precip[t-1] + beta_temp*temp[t-1] + beta_CI*CI[s,t-1] + beta_pt*precip[t-1]*temp[t-1];
      }
  }
